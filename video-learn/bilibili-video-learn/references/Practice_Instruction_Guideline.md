...
1. **Write the question as an imperative numbered task list**, one action per step, with nested sub-steps for detail.
...
2. **Always define every name for the respondent.** Function names, method names, struct names, field names, variable names, receiver types, file names, route names, migration names. Never make them invent an identifier.
   - ✅ "Create a reusable `writeJSON()` helper method"
   - ❌ "Create a helper to write JSON"

3. **Always give the exact file path** the code goes into, either in the step (`in cmd/api/helpers.go`) or as a `File: cmd/api/helpers.go` header line above the code block in the solution.

4. **Always spell out the signature** — parameter names, parameter types, and return types.
...
5. **Always give the exact literal values.** Never let the respondent guess a number or string:
...
6. **Always give setup/dependency instructions as their own step**, and show the exact command in the solution. The respondent must never have to look up an install command.
...
7. **Give the data setup, not just the query.** If a task needs rows in the DB or a specific app state, supply the exact `curl`/`BODY='...'`/SQL that creates it.

8. **End every practical block with a verification step**, and put the exact verification command _and its expected output_ in the solution.
